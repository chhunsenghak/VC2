    ...
    <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-200">
      <div class="container mx-auto px-6 py-2">
        <div class="text-right">
          @can('Post create')
          <a href="{{route('admin.durationtype.create')}}" class="bg-blue-500 text-white font-bold px-5 py-1 rounded focus:outline-none shadow hover:bg-blue-500 transition-colors ">Create New</a>
          @endcan
        </div>
            ...
            <tbody>
              ...
              <tr class="hover:bg-grey-lighter">
                ...
                <td class="py-4 px-6 border-b border-grey-light text-right">
                  @can('Permission access')
                  <a href="{{route('admin.durationtype.edit',$item->id)}}" class="bg-yellow-500 font-bold py-1 px-3 rounded text-xs bg-green hover:bg-green-dark text-white">Edit</a>
                  @endcan

                  @can('Permission access')
                  <form action="{{ route('admin.durationtype.destroy', $item->id) }}" method="POST" class="inline">
                    @csrf
                    @method('delete')
