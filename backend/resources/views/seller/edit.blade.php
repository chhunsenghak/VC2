        ...
        <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-200">
            <div class="container mx-auto px-6 py-1 pb-16">
                <div class="bg-white shadow-md rounded my-6 p-5">
                    <form method="POST" action="{{ route('admin.seller.update',$user->id)}}" enctype="multipart/form-data">
                        @csrf
                        @method('put')
                        ...
                        <div class="flex gap-4 mt-5">
                            <div class="flex flex-col w-1/2">
                                ...
                                <input id="name" type="text" name="name" value="{{ old('user',$user->name) }}" placeholder="Enter name" class="px-4 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-200" />
                            </div>
                            <div class="flex flex-col w-1/2">
                                <label for="description" class="text-gray-700 select-none font-medium">Email</label>
                                <input id="description" type="text" name="description" value="{{ old('email',$user->email) }}" placeholder="Enter description" class="px-4 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-200" />
                            </div>
                        </div>

                        <div class="flex gap-4 mt-5">
                            <div class="flex flex-col w-1/2">
                                <label for="price" class="text-gray-700 select-none font-medium">Check</label>
                                <input id="price" type="text" name="price" value="{{ old('check', $user->check ) }}" placeholder="Re-enter password" class="px-4 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-200" />
                            </div>
                            <div class="flex flex-col w-1/2">
